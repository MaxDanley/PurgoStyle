...
import { sendOrderNotificationToSupport, sendZellePaymentInstructions } from "@/lib/email";
import { validateGuestOrderInfo } from "@/lib/validation";
import { handleCheckoutSubscription } from "@/lib/subscriber-helpers";
import { cookies } from "next/headers";

/**
 * Create an order with Zelle payment
 * This creates the order in PENDING status and provides Zelle payment instructions
 */
export async function POST(req: Request) {
  try {
    ...
    console.log("🛒 Creating Zelle order:", { items: items.length, shippingInfo });
    ...
    // Apply 5% discount for Zelle payments (on subtotal + shipping + insurance)
    ...
    if (orderWithItems) {
      try {
        await sendZellePaymentInstructions(
          validatedInfo.email,
          order.orderNumber,
          total,
          {
        ...
        console.log("📧 Payment instructions email sent to customer for Zelle order");
      } catch (emailError) {
    ...
    if (orderWithItems && orderWithItems.shippingAddress) {
      try {
        ...
        console.log("📧 Support notification email sent for Zelle order");
      } catch (emailError) {
    ...
    console.log("✅ Zelle order created successfully:", order.orderNumber);
    ...
    console.error("❌ Zelle order creation failed:", error);
    return NextResponse.json(
