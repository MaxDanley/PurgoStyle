...
export default function ZellePaymentForm({
  total: _total,
  items,
  shippingInfo,
  billingInfo,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Create Zelle order
      const response = await fetch("/api/orders/create-zelle", {
        method: "POST",
        headers: {
      ...
      console.error("Zelle order creation error:", error);
      setPaymentError("Something went wrong. Please try again shortly or contact support");
      setIsProcessing(false);
    }
  };
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          <p className="text-sm text-blue-800 mb-2">
            <strong>Zelle Payment Instructions:</strong> Place your order first then follow instructions to pay with Zelle.
          </p>
          <p className="text-sm text-blue-800">
            Your order will not be processed until funds have cleared in our Zelle/Bank account.
          </p>
        </div>

        <div className="space-y-2">
          <label htmlFor="zelle-research-confirmation" className="flex items-start gap-3 text-sm text-gray-700">
            <input
              id="zelle-research-confirmation"
              type="checkbox"
