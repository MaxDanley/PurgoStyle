-- Two orders for 2/12/26 with exact totals and discount codes applied.
-- Run in Supabase SQL Editor after products exist (e.g. from seed-purgo-products.sql).
--
-- Order 1: 2/12/26 11:40 AM — $247.36 (•••• 8046)
--   Items: Black Sweatpants (M $125) + Black Logo T-Shirt Front (M $100) = $225 subtotal
--   Shipping $25 + Insurance $3.50 - Discount $6.14 = $247.36
--
-- Order 2: 2/12/26 9:55 AM — $205.97 (•••• 9160)
--   Items: Black Sweatpants (L $125) + White T-Shirt Infinity Logo (L $100) = $225 subtotal
--   Shipping $0 + Insurance $3.50 - Discount $22.53 = $205.97
...
-- 2) Shipping addresses (guest orders)
INSERT INTO "Address" (id, "userId", name, street, apartment, city, state, "zipCode", country, phone, "isDefault", "createdAt", "updatedAt")
VALUES
  ('addr-order-24736', NULL, 'Customer One', '123 Main St', NULL, 'Phoenix', 'AZ', '85001', 'US', NULL, false, '2026-02-12 11:40:00', '2026-02-12 11:40:00'),
  ('addr-order-20597', NULL, 'Customer Two', '456 Oak Ave', NULL, 'Scottsdale', 'AZ', '85251', 'US', NULL, false, '2026-02-12 09:55:00', '2026-02-12 09:55:00')
...
SELECT
  ...
  '2026-02-12 11:40:00',
  '2026-02-12 11:40:00'
WHERE NOT EXISTS (SELECT 1 FROM "Order" WHERE "orderNumber" = 'PL-20260212-1140');
...
SELECT
  ...
  '2026-02-12 09:55:00',
  '2026-02-12 09:55:00'
WHERE NOT EXISTS (SELECT 1 FROM "Order" WHERE "orderNumber" = 'PL-20260212-0955');
...
VALUES
  (gen_random_uuid()::text, 'order-24736', 'PENDING', 'Order placed', '2026-02-12 11:40:00'),
  (gen_random_uuid()::text, 'order-24736', 'PROCESSING', 'Payment received', '2026-02-12 11:40:00'),
  (gen_random_uuid()::text, 'order-20597', 'PENDING', 'Order placed', '2026-02-12 09:55:00'),
...
-- If orders PL-20260212-1140 / PL-20260212-0955 already exist, skip or delete those orders before re-running.
