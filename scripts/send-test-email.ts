...
const fromEmail = process.env.EMAIL_FROM || "noreply@purgolabs.com";

async function sendTestEmail() {
  try {
    // Read the HTML template
    const htmlPath = path.join(process.cwd(), "www.purgolabs.com", "email.html");
    let htmlContent = fs.readFileSync(htmlPath, "utf-8");

    // Read all images and convert to base64
    const imagesDir = path.join(process.cwd(), "www.purgolabs.com", "images");
    const imageFiles = [
    ...
    // Send the email
    console.log("\n📧 Sending test email to support@purgolabs.com...");
    
    const result = await resend.emails.send({
      from: fromEmail,
      to: "support@purgolabs.com",
      subject: "Test Email - Purgo Labs Template",
      html: htmlContent,
    });
